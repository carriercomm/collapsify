  ...
  return Rx.Observable.create(function(observer) {
    ...
    return scheduler.scheduleRecursiveWithState({
      ...
      if (self.forbidden.test(currentURL)) {
        return observer.onError(new Error('Forbidden URL'));
      }
      ...
      needle.request('GET', currentURL, null, {
        ...
        if (res.statusCode >= 400) {
          return observer.onError(httperr[res.statusCode](url));
        }
        ...
        if (res.statusCode >= 300) {
          ...
          return observer.onError(new Error('Redirection Limit Hit: ' + originalURL));
        }
